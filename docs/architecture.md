...
VitePress 管理全站主题偏好，首屏脚本把 `.dark` 同步到根节点 `data-theme`。示例可使用独立主题；Dialog 将主题与品牌显式传至 Portal 内容。Tailwind 不加载 Preflight，扫描限定示例组件；utilities 不放入命名层，以免被 VitePress 未分层的基础规则覆盖。页面样式和控件样式分开，正文不使用大号控件 token。
