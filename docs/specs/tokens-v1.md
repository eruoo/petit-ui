...
> 范围说明：下文“本次”及“本轮验收”记录 CSS 核心的已完成交付。后续已授权建立 `site/`，其现状与验证方式见[项目架构](../architecture.md)和[开发指南](../development.md#文档站开发与验证)；该授权不改变 token 包边界。

## 目标与范围

交付 28 个语义颜色、4 个圆角、2 个边框宽度、2 个字重和 3 个字号，以及普通 CSS 和 Tailwind CSS 4 两个消费入口。按钮、输入框和卡片使用同一套变量，支持浅色、深色、局部主题及用户覆盖。

本次复用已有开发依赖，不新增运行时依赖、构建工具、配置生成器或第三方服务。组件库、完整基础色阶、站点、Reka UI 示例包、JavaScript 主题管理和 OIDC 不在本次实现范围内。

默认视觉转译星布谷地公开界面的奶油纸色、深褐文字、青灰衬底、金黄操作、厚奶油边缘与赭色选择轮廓。颜色值属于可调整的默认主题，语义名称与含义属于公共接口。第一版保持两个样式文件，不增加独立主题包或多套品牌预设。

## 风格依据与网页转译

依据 2026-09-21 的[官方 Final Beta 更新公告](https://www.hoyolab.com/article/46776026)中的四张公开界面图。S03–S06 是本项目的证据编号；不是客户端构建号。以下结论来自公开截图，不代表取得了官方设计 token 或资产授权。

| 证据                          | 可观察的关系                                           | 本包及消费示例的转译                                                          |
| ----------------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------- |
| S03，公告第 4 张图，烹饪界面  | 青灰卡片、厚奶油框、赭色选中边、纸色详情、金黄深字操作 | 核心浅色依据：分开阅读面与衬底，保留颜色关系和圆润轮廓。                      |
| S04，第 7 张图，蓝图详情      | 浅绿背景、深褐标题、预览与说明分区、深色胶囊操作       | 辅助青灰／纸色分区与文字层级；次级按钮可用 foreground 填充、background 文字。 |
| S05，第 9 张图，星海地图      | 夜蓝地图、奶油操作、当前区域亮边                       | 只支持局部地图观察，不能推出官方完整深色主题。                                |
| S06，第 15 张图，Omnidex 入口 | 深色胶囊标题、较小的进度行、圆形图标底                 | 标题与辅助文字拉开字号／字重，胶囊操作与圆形标记保留简洁轮廓。                |
...
网页采用 `#f6f3e7` 页面底和取样纸色 `#f5f4e0` 阅读面，把深底板色转作正文，并独立保留 `#564630` 标题褐色。选择外框恢复 `#b77a32`，依靠相邻的奶油内框／纸色背景识别，不再把它直接放在青灰上要求 3:1。其余 hover、active、链接、状态色和功能边缘是网页设计值。深色使用暖炭、灰绿、奶油文字和相同金黄关系，是本项目的扩展主题，不是官方深色主题复刻。
...
- 基础色值由下表维护，并直接写入各主题的语义声明。第一版不额外导出数字色阶，也不引入可配置的基础变量层。
...
| 语义名                  | 浅色      | 深色      | 用途                                                                 |
| ----------------------- | --------- | --------- | -------------------------------------------------------------------- |
| `background`            | `#f6f3e7` | `#252a28` | 页面背景。                                                           |
| `surface`               | `#f5f4e0` | `#323833` | 卡片、输入框等内容表面。                                             |
| `surface-hover`         | `#eee8d8` | `#40483f` | 中性控件悬停表面。                                                   |
| `surface-active`        | `#e3ddcb` | `#4b5348` | 中性控件按下或选中表面。                                             |
| `foreground`            | `#3a3432` | `#f6f3e7` | 正文、标题、主要图标。                                               |
| `foreground-muted`      | `#686052` | `#d0c7b0` | 辅助文字及输入提示文字。                                             |
| `foreground-disabled`   | `#9a907d` | `#8d9487` | 真正不可交互的禁用内容。                                             |
| `border`                | `#fdf8e1` | `#bfb69b` | 奶油色装饰边；不承担功能边界。                                       |
| `border-strong`         | `#71634e` | `#b2a58b` | 需要辨认的输入框等控件边界。                                         |
| `focus`                 | `#285e75` | `#b3d9db` | 焦点轮廓；样例与控件之间留出背景色间隔。                             |
| `primary`               | `#f3c75d` | `#f3c75d` | 金黄主操作填充；不用于常规表面的小号文字。                           |
| `primary-hover`         | `#e8b645` | `#ffda7e` | 主操作悬停背景。                                                     |
| `primary-active`        | `#e5af47` | `#e8b645` | 主操作按下背景。                                                     |
| `on-primary`            | `#564630` | `#564630` | 三种主操作背景上的文字。                                             |
| `success`               | `#365e45` | `#a6c99f` | 成功状态背景及常规表面上的状态文字。                                 |
| `on-success`            | `#fffcf2` | `#252a28` | 成功状态背景上的文字。                                               |
| `warning`               | `#805514` | `#ebc578` | 警告状态背景及常规表面上的状态文字。                                 |
| `on-warning`            | `#fffcf2` | `#252a28` | 警告状态背景上的文字。                                               |
| `error`                 | `#9b3d34` | `#f0aba0` | 错误状态背景及常规表面上的错误文字。                                 |
| `on-error`              | `#fffcf2` | `#252a28` | 错误状态背景上的文字。                                               |
| `surface-accent`        | `#9cc2c3` | `#385657` | 青灰衬底、选择区域，搭配 foreground。                                |
| `border-selected`       | `#b77a32` | `#edc17a` | 赭色选择轮廓，配合勾选或文字。                                       |
| `link`                  | `#986323` | `#edc17a` | 常规表面上的文字链接，常态带下划线。                                 |
| `foreground-heading`    | `#564630` | `#f6f3e7` | 粗标题及重要标签；与正文、辅助字分层。                               |
| `foreground-accent`     | `#a97f1b` | `#edc17a` | 常规表面上的大号金色强调文字；只保证 3:1，不用于普通正文或小号链接。 |
| `surface-accent-strong` | `#85b6bc` | `#30494b` | 青灰较深分区，搭配 foreground；不放浅褐标题。                        |
| `surface-accent-soft`   | `#c7dcd6` | `#48645e` | 媒体圆底，搭配 foreground 或 foreground-heading。                    |
| `surface-band`          | `#e5d8ba` | `#4e5142` | 纸色面板上的标题带和低对比装饰层。                                   |
...
圆角与下节的边框／排版数值不随主题变化，在根节点声明一次，由后代继承。切换局部主题不重置已有的这些覆盖。
...
| `--petit-font-weight-display`   | `900`      | 主标题、重要标签与主操作。                 |
| `--petit-font-weight-strong`    | `800`      | 次级标签、辅助层级；不把所有字都设为最粗。 |
...
导入入口不会设置元素字体。字号、字重是否适合实际字体需由消费方检查；普通长文仍由使用方设置合适的正文尺度。
...
- `:where([data-theme='dark'])` 声明全部深色颜色，排列在浅色规则之后。
...
1. 根节点未设置主题时使用浅色，不自动跟随操作系统。
2. 根节点设置 `data-theme="dark"` 后使用深色；设置 `light` 恢复浅色。
3. 后代可设置自己的 `data-theme`，形成独立颜色作用域。嵌套的 `light` 能恢复浅色，嵌套的 `dark` 能切回深色。
4. 未识别的属性值不匹配主题规则，后代继承最近作用域的值；根节点仍有浅色默认值。
5. 主题属性只改变变量，不自动给元素应用样式。存储用户偏好、系统主题监听及切换按钮由应用负责。
6. 同一作用域可直接覆盖语义变量，普通后代继承该覆盖。后代显式主题会重新声明颜色；要保持自定义品牌，应在该主题节点或其内部也应用品牌覆盖类。
7. 主色、hover、active、on-primary 和 focus 是独立值，修改主色不会自动推导其余值。

全局主题建议设置在 `html` 上，让挂载到 `body` 的弹出层也继承。局部主题不会自动跨越 DOM 上的 Portal/Teleport 边界。
...
- 颜色表中每个语义名 `X` 对应 `--color-petit-X: var(--petit-color-X)`，共 28 项。
- 圆角表中每个级别 `X` 对应 `--radius-petit-X: var(--petit-radius-X)`，共 4 项。
- 边框宽度映射 `--border-width-petit-X`，得到 `border-petit-frame`／`border-petit-selected`；字体字重映射 `--font-weight-petit-X`，得到 `font-petit-display`／`font-petit-strong`；字号映射 `--text-petit-X`，得到 `text-petit-heading`／`text-petit-label`／`text-petit-action`。总计 39 项映射。
- 不导入 Tailwind 本体、Preflight 或其他外部 CSS，不清空默认命名空间，不改写 `dark` variant。
...
主题切换直接改变语义变量，同一个工具类随作用域改变颜色，不需要为这些颜色再写一组 `dark:` 类。`@theme inline` 使编译后的工具类直接引用语义变量，保留元素所在作用域的覆盖效果。[Tailwind 官方说明](https://tailwindcss.com/docs/theme#referencing-other-variables)
...
README 提供两个真实导入入口、主要角色说明、明暗切换、局部覆盖、品牌状态值独立、禁用颜色用途，以及当前 alpha API 状态。维护者尚未选择许可证，本次不自行添加 LICENSE、`license` 字段或许可声明；许可证选择由维护者在后续功能版本发布前处理，不阻塞本地实现。
...
- 解析真正的 `src/tokens.css`，验证普通入口没有 Tailwind 指令、导入、常规样式声明或运行时依赖；浅深两套颜色的公共名称一致，圆角值完整。
- Tailwind fixture 导入真正的适配文件，不再只验证 fixture 自己定义的假 token。保留 `source(none)` 与显式 source，隔离工作区扫描影响。
- 覆盖背景、文字、强边框、焦点轮廓、圆角、hover/active/disabled 及 `bg-petit-primary/50` 的编译。核心断言是生成正确 CSS 属性、引用语义变量且不存在未展开的指令。
...
浅色和深色分别验证下列组合，共 90 组；浅色选择框与奶油边另检查一组，总计 91 组。
...
| `border-selected`                       | `background`、`surface`；浅色另包含 `border`                                     | `3:1`    |
...
浅色选中外框紧贴奶油内框，与其达到 3:1；不把选择框直接当青灰衬底上的文字／细线。深色选中圈相对于深色页面或表面识别，仍保留非颜色选择语义与独立焦点。纹样为装饰，不承担状态识别。源图浅色辅助字在网页样例中使用可读的 `foreground-muted`，不照搬其低对比取样；这一差异应在视觉对照中明确显示。
...
使用系统临时目录中的同尺度 S03 对照页，以及普通 CSS 和实际 Tailwind 编译产物的技术消费页。保持原生 HTML 语义；不用 Vue、Reka UI 或 VitePress 来完成本轮验收。

1. 未设置主题、根节点浅色、根节点深色分别呈现对应默认值。
2. 深色页面内的浅色区域、浅色区域内再嵌套深色，以及未识别的属性值均符合继承约定。
3. 覆盖主色、on-primary 和圆角只影响指定作用域；主题边界对颜色与圆角的重置行为符合本规范。
...
7. 保存明暗效果及关键控件状态的截图或检查记录，并报告实际使用的浏览器。浏览器不可用时，如实列出缺口，不能仅凭 CSS 编译成功宣称全部通过。

浏览器样例和一次性打包消费脚本保持临时，不新增站点、长期 demo 包或仅服务本次验收的工具依赖。
...
字体实测：标题 Nunito Black，Cook 与 Details 为 Nunito ExtraBold，中文为 Noto Sans SC Black。Details 实际 26px／800，浅色纸面对比度 3.2933474744411275:1，深色 7.152225534951918:1；普通正文及链接仍按 4.5:1 检查。
...
- [Tailwind dark mode](https://tailwindcss.com/docs/dark-mode)：data attribute 的使用方集成方式。
...
- [S05：三测星海地图](https://upload-os-bbs.hoyolab.com/upload/2026/09/21/9bb4a04113abc093c557a865a2d62f65_1727392156795378942.png?x-oss-process=image%2Fresize%2Cs_1000%2Fauto-orient%2C0%2Finterlace%2C1%2Fformat%2Cwebp%2Fquality%2Cq_100)
