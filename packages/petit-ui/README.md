...
与框架无关的 CSS design tokens，提供可选的 Tailwind CSS 4 适配入口。包含 29 个语义颜色、4 个圆角、2 个边框宽度、2 个字重和 3 个字号，不包含组件、reset 或 JavaScript 运行时。

默认浅色转译星布谷地公开界面的奶油纸色、深褐文字、青灰衬底、金黄操作、厚奶油边缘和赭色选择轮廓。颜色来自截图观察与独立网页设计，不是官方 token；未包含游戏图片或字体。
...
<article class="bg-petit-surface text-petit-foreground rounded-petit-lg">
  <button
    type="button"
    class="bg-petit-primary text-petit-on-primary rounded-petit-full border-0 px-6 py-3 text-petit-action font-petit-strong enabled:hover:bg-petit-primary-hover enabled:active:bg-petit-primary-active focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-petit-focus disabled:bg-petit-surface-hover disabled:text-petit-foreground-disabled disabled:cursor-not-allowed"
  >
    保存
  </button>
</article>
...
适配入口自动导入 tokens，保留 Tailwind 默认主题和 variants。`bg-petit-primary/50` 等透明度修饰可直接使用。同一工具类可直接使用所在作用域的品牌覆盖。

上面的按钮通过原生 `disabled` 属性禁用；颜色不会自动阻止交互。`enabled:hover:` 和 `enabled:active:` 排除禁用状态。使用 `aria-disabled` 或无样式库的 `data-disabled` 时，由消费方同时处理样式和激活行为。

## 主题与覆盖

包只提供浅色默认值，不跟随系统外观。导入后根节点即可使用；局部设置 `data-theme="light"` 可恢复默认颜色。变量本身不会给元素设置颜色或 `color-scheme`。

```html
<section class="brand-scope">
  <article>继承品牌配色</article>
  <article data-theme="light">恢复默认颜色</article>
</section>
```
...
普通后代继承覆盖；后代的 `data-theme="light"` 重新声明默认颜色，但保留继承的圆角、边框宽度和排版数值覆盖。需要再次使用品牌配色时，在该节点或其内部应用品牌类。主色、hover、active、on-primary 和 focus 相互独立，覆盖主色不会自动推导其他值。其他 `data-theme` 值不匹配规则，继续继承。局部覆盖不会跨越 DOM 上的 Portal/Teleport 边界。
...
- `border` 是奶油色装饰边，可用较厚边缘呈现卡片；需要靠边缘辨认的输入等控件使用 `border-strong`。金黄主操作不强制附加深褐边框。焦点使用 `focus`，建议至少 2px 轮廓和 2px offset。
...
- `surface-accent` 是青灰衬底，搭配 `foreground` 正文；辅助、链接和状态文字留在常规阅读面。`border-selected` 是赭色选择轮廓，配勾选或“已选”文字；外框紧贴奶油内框，键盘焦点另用 `focus` 表达。
- `scrim` 用于模态内容背后的暗化遮罩，CSS 为 `--petit-color-scrim`，Tailwind 为 `bg-petit-scrim`。它已包含透明度，`/50` 会将现有 alpha 再减半；内容面仍用 `surface`。局部品牌覆盖必须应用到 Portal 的遮罩元素，不能只应用到内容面。
...
功能 alpha 提供风格角色及有限的常见状态配方，不规定完整正文尺度、控件密度或所有交互变体。主操作色也可用于强调选中，配合勾或文字；破坏性文字操作可用 `error` 配 `surface`，悬停用 `surface-hover`，按下改为 `error/on-error` 实色配对。不要把小号 `link` 任意放在中性状态底；其他组合需自行验证。字段校验、菜单和浮层的组合见[消费配方](https://github.com/eruoo/petit-ui/blob/main/site/guide/states.md)（对应源码版本），允许的颜色组合以[规范](https://github.com/eruoo/petit-ui/blob/main/docs/specs/tokens-v1.md#常见-ui-状态合同)为准。

仅导出 `petit-ui/tokens.css`、`petit-ui/tailwind.css` 和 `petit-ui/package.json`，不提供包根或源码深层路径导出。

## 风格依据与消费样式

浅色核心依据是[官方 Final Beta 公告](https://www.hoyolab.com/article/46776026)第 4 张烹饪界面的纸色面板、青灰卡片、奶油厚边、赭色选框和金黄按钮；第 7 张蓝图详情与第 15 张 Omnidex 入口补充深褐标题、圆形标记和胶囊操作的层级关系。取样色、网页调整与来源边界详见[项目规范](https://github.com/eruoo/petit-ui/blob/main/docs/specs/tokens-v1.md#风格依据与网页转译)（对应源码版本）。
